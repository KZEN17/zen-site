...
import { getAllCombosWithRates } from '@/lib/db/combos'
...
export default async function RoomsPage() {
  const [rawRooms, rawCombos] = await Promise.all([
    getAllRoomsWithRates(false),
    getAllCombosWithRates(false).catch(() => [] as RoomComboWithRates[]),
  ])

  const rooms: RoomWithRates[] = JSON.parse(JSON.stringify(rawRooms))
  const combos: RoomComboWithRates[] = JSON.parse(JSON.stringify(rawCombos))
  ...
  return (
    <div className="space-y-10">
            ...
            <div key={room.$id} className="bg-white rounded-xl shadow-sm p-5">
              ...
              {room.rates.length > 0 ? (
                ...
                <p className="mt-2 text-xs text-gray-400 italic">
                  No rate tiers — add them on the edit page.
                </p>
              )}
            </div>
          ))}
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Room Combinations</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            Pre-defined groups used when booking multiple rooms together
          </p>
        </div>
        <ComboManager rooms={rooms} initialCombos={combos} />
      </section>
    </div>
  )
}
