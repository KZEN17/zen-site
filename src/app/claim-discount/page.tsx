          ...
          <div className="p-6">
            ...
            {!isRevealed ? (
              ...
              <div className="space-y-4">
                {/* Step 1: Copy code */}
                <div className={`border rounded-xl p-4 ${copied ? 'bg-green-50 border-green-300' : 'bg-amber-50 border-amber-200'}`}>
                  <p className={`text-sm font-medium mb-2 text-center ${copied ? 'text-green-700' : 'text-amber-800'}`}>
                    {copied ? 'Copied! Now send us a message' : 'Step 1: Copy your discount code'}
                  </p>
                  <button
                    onClick={handleCopyCode}
                    className={`w-full py-3 px-4 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 ${copied ? 'bg-green-500 hover:bg-green-600' : 'bg-amber-500 hover:bg-amber-600'}`}
                  >
                    {copied ? (
                    ...
                    {copied ? 'Copied!' : `Copy Code: ${discountCode}`}
                  </button>
                </div>
